...
class UserViewTest(APITestCase):
  ...
  def test_should_create_user(self):
    response = self.client.post('/users/',{"username": "novo_user", "email": "novo_user@email.com", "password": "12345678", "phone": "+5519999994512", "is_master": False})
  ...
  def test_should_login_as_superuser(self):
    response = self.client.post('/login/', {"email": self.superemail, "password": self.superpassword})
    self.assertEqual(response.status_code, status.HTTP_200_OK)
    user = response.data
    self.assertTrue(user['refresh'])
    self.assertTrue(user['access'])
  ...
  def test_should_update_user_with_own_token(self):
    response_login = self.client.post('/login/', {"email": self.email, "password": self.password})
    token = response_login.data['access']
    self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + token)
    response_update = self.client.patch('/users/' + str(self.user.id) + '/', {'username': "Updated_Username", 'password': "1234"})
    self.assertEqual(response_update.status_code, status.HTTP_200_OK)
    
  def test_should_update_user_with_superuser_token(self):
    response_login = self.client.post('/login/', {"email": self.superemail, "password": self.superpassword})
    token = response_login.data['access']
    self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + token)
    response_update = self.client.patch('/users/' + str(self.user.id) + '/', {'username': "Updated_Username", 'password': "1234"})
    self.assertEqual(response_update.status_code, status.HTTP_200_OK)
  ...
  def test_should_not_update_is_master(self):
    ...
    token = response_login.data['access']
    self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + token)
    response_update = self.client.patch('/users/' + str(self.user.id) + '/', {"is_master": True})
